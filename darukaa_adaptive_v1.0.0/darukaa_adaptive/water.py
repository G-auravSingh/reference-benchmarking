...
class WaterDetector:
    ...
    def water_mask_for_period(self, geometry, start: str, end: str):
        ...
        if n >= self.config.water.min_primary_images:
            ...
            mask = probability.gte(self.config.water.primary_probability_threshold).rename("water")
        ...
        n1 = self._size(s1)
        if n1 < 1:
            return ee.Image(0).selfMask().rename("water"), "none", 0

        detections = s1.map(
            lambda img: img.lt(self.config.water.fallback_vv_threshold_db).rename("water")
        )
        occurrence = detections.mean()
        mask = occurrence.gte(self.config.water.fallback_majority_fraction).rename("water")
    ...
    def occurrence_image(self, geometry, start: str, end: str):
        ...
        if n >= 1:
            occurrence = (
                dw.select("label")
                .map(lambda img: img.eq(0).rename("water"))
                .mean()
        ...
        if self.config.water.fallback_enabled:
            ...
            if n1 >= 1:
                occurrence = (
                    s1.map(lambda img: img.lt(self.config.water.fallback_vv_threshold_db).rename("water"))
                    .mean()
                    .rename("water_occurrence")
                )
                return occurrence, "sentinel1_vv", n1

        return ee.Image(0).selfMask().rename("water_occurrence"), "none", 0
    ...
    def _area(self, geometry, mask, scale=10) -> Optional[float]:
        ...
        if value is None:
            return None
        return float(ee.Number(value).getInfo() / 1e4)

    def area_summary(self, geometry, start: str, end: str) -> Dict[str, Any]:
        import ee

        boundary_area = self._area(geometry, ee.Image(1).selfMask(), 10)
        mask, method, n = self.water_mask_for_period(geometry, start, end)
        water_ha = self._area(geometry, mask, 10)
        fraction = None if water_ha is None or not boundary_area else water_ha / boundary_area * 100.0

        occurrence, _, _ = self.occurrence_image(geometry, start, end)
        ...
        occurrence_value = None if occurrence_value is None else float(occurrence_value.getInfo())

        status = "ok" if water_ha is not None else "insufficient_data"
        return WaterPeriodResult(
            period_start=start,
            period_end=end,
            period_end_inclusive=(str(__import__("datetime").date.fromisoformat(end) - __import__("datetime").timedelta(days=1)) if end else None),
            images_used=n,
            method=method,
            ...
            water_present=(fraction is not None and fraction >= self.config.water.water_presence_area_fraction_threshold),
            status=status,
    ...
    def persistence(self, geometry, start: str, end: str) -> Dict[str, Any]:
        ...
        occurrence, method, n = self.occurrence_image(geometry, start, end)
        value = occurrence.reduceRegion(
            reducer=ee.Reducer.mean(), geometry=geometry, scale=10, maxPixels=1e8, bestEffort=True
        ).get("water_occurrence")
        value = None if value is None else float(value.getInfo())
        return {
            "period_start": start,
            "period_end": end,
            "method": method,
