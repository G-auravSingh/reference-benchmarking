...
class ReferenceSelector:
    ...
    def _compute_tier2(
        ...
        # Step 4: HMI image (current asset by default; legacy asset opt-in for reproduction)
        if getattr(cfg, "use_legacy_hmi_asset", False):
            ...
            hmi_src = ee.ImageCollection(cfg.hmi_gee_asset)
            hmi = hmi_src.first().select(cfg.hmi_gee_band) if hasattr(hmi_src, "first") else \
                  ee.Image(cfg.hmi_gee_asset).select(cfg.hmi_gee_band)
