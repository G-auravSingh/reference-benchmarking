...
| forest_loss_rate | Hansen GFC 2025 v1.13 (current); windows were hardcoded, now `config.forest_loss_windows` (configurable) with `config.forest_loss_primary_window` pinning which window scores (anti-cherry-pick, deliberate) | Fixed |
