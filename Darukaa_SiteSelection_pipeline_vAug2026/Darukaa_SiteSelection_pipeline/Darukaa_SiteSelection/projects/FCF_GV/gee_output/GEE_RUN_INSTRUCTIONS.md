...
3. Name the asset (suggested: `fcf_gv_candidates`),
   wait for the upload task to finish (Tasks tab, yellow -> green)
4. Open gee_output/FCF_GV_ready_to_run.js — this is a ready-to-paste copy of the
...
6. Set EXPORT_NAME = "gee_covariates_output" and
   DRIVE_FOLDER = "fcf_gv_gee" if not already set.
7. Run the script, open the Tasks tab, click Run on the export task
8. Once complete, download the CSV from Google Drive
9. Place it at: projects/FCF_GV/gee_output/gee_covariates_output.csv
10. Re-run: python covariate_ingest.py projects/FCF_GV
