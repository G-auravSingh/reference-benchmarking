...
3. Name the asset (suggested: `soulforest_veltoor_candidates`),
   wait for the upload task to finish (Tasks tab, yellow -> green)
4. Open gee_output/SoulForest_Veltoor_ready_to_run.js — this is a ready-to-paste copy of the
...
6. Set EXPORT_NAME = "gee_covariates_output" and
   DRIVE_FOLDER = "soulforest_veltoor_gee" if not already set.
7. Run the script, open the Tasks tab, click Run on the export task
8. Once complete, download the CSV from Google Drive
9. Place it at: projects/SoulForest_Veltoor/gee_output/gee_covariates_output.csv
10. Re-run: python covariate_ingest.py projects/SoulForest_Veltoor
