# GEE run instructions — FCF_Soova

Archetype: agroforestry (scattered)

1. Go to code.earthengine.google.com
2. Assets tab -> NEW -> Table Upload -> Shapefile
   -> select gee_output/FCF_Soova_gee_upload.zip
...
3. Name the asset (suggested: `fcf_soova_candidates`),
   wait for the upload task to finish (Tasks tab, yellow -> green)
4. Open gee_output/FCF_Soova_ready_to_run.js — this is a ready-to-paste copy of the
...
6. Set EXPORT_NAME = "gee_covariates_output" and
   DRIVE_FOLDER = "fcf_soova_gee" if not already set.
7. Run the script, open the Tasks tab, click Run on the export task
8. Once complete, download the CSV from Google Drive
9. Place it at: projects/FCF_Soova/gee_output/gee_covariates_output.csv
10. Re-run: python covariate_ingest.py projects/FCF_Soova
